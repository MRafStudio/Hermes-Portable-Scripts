...
- **НЕ править `.bat` через heredoc** (`python - <<'EOF'`): экранирование искажается
  (двойные бэкслеши становятся одинарными, а `\r`, `\v` внутри путей превращаются в
  управляющие символы CR/VT), пути рвутся, файл ломается — и это не видно до запуска.
  Единственно верный путь: создать фиксер отдельным файлом через `write_file`, затем
