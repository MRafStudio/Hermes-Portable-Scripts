...
if exist "%DATA_DIR%\llm\default_model.cfg" (
    for /f "tokens=1,* delims==" %%a in ('findstr /b "MODEL_FILE MMPROJ_FILE MAXCTX MODEL_ID" "%DATA_DIR%\llm\default_model.cfg"') do (
...
"%NSSM_EXE%" install "%SERVICE_NAME%" "%LLAMA_EXE%" -m "%MODELS_DIR%\%MODEL_FILE%" --mmproj "%MODELS_DIR%\%MMPROJ_FILE%" --alias llama/%MODEL_FILE:~0,-5% -c %MAXCTX% !SERVER_FLAGS! -ngl 999 --flash-attn 1 --parallel 1 --port %LLAMA_PORT% --host 127.0.0.1
if errorlevel 1 (
