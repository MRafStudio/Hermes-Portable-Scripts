...
if exist "%CFG_FILE%" (
    for /f "tokens=1,* delims==" %%a in ('findstr /b "MODEL_FILE MMPROJ_FILE MAXCTX MODEL_ID" "%CFG_FILE%"') do (
...
REM ДУМАНИЕ (reasoning) ВСЕГДА ВКЛЮЧЕНО нативно (llama-server по умолчанию) — БЕЗ --reasoning off!
REM КРИТИЧНО для агентских циклов (tool-calling) и кристаллизации памяти MemOS: выключение думания
REM ломает function calling (модель имитирует вызовы текстом). НЕ возвращать флаг --reasoning off.
REM Отвязка: llama-server — отдельный процесс (start /min), окно свернуто, лог — в своё окно
start "LlamaCPP %MODEL%" /min "%LLAMA_DIR%\llama-server.exe" -m "%MODEL_PATH%" --mmproj "%MMPROJ%" --alias llama/%MODEL:~0,-5% -c %MAXCTX% %SERVER_FLAGS% -ngl 999 --flash-attn 1 --parallel 2 --port %PORT% --host 127.0.0.1
