...
# kv_quant — квантование KV-кэша (--cache-type-k/v): q8_0 даёт лучшее качество длинного контекста,
...
def main():
    ...
    if cmd == "list":
        ...
        # для set /p в Start-llama.bat и для nssm AppParameters)
        mid = int(sys.argv[2]) if len(sys.argv) > 2 else 0
        for m in MODELS:
            if m[I_ID] == mid:
