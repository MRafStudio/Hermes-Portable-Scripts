...
lines = [
    ...
    '"%LLAMA_DIR%\\llama-server.exe" -m "%MODEL_PATH%" --mmproj "%MMPROJ%" --alias llama/%MODEL:~0,-5% -c ' + max_ctx + ' -ngl 999 --flash-attn 1 --parallel 1 --image-min-tokens 1024 --port %PORT% --host 127.0.0.1',
    'pause',
]
content = '\r\n'.join(lines) + '\r\n'
