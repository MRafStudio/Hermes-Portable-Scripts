...
REM Глобальный model.base_url снимаем (перекрывает provider-овский!)
"%HERMES_BIN%" config unset model.base_url >nul 2>&1
echo   %ESC%[1;32m+ %ESC%[0m Глобальный model.base_url снят (используется providers.deepseek-hr.base_url)
    exit /b 0
)
...
echo(%HR_PROV%|findstr /C:"deepseek-hr" >nul 2>&1 && set "HR_ACTIVE=1"
if "!HR_ACTIVE!"=="1" (
    call :hr_disable_proxy
) else (
    call :hr_enable_proxy
...
"%HEADROOM_NSSM%" set Headroom AppEnvironmentExtra "HEADROOM_WORKSPACE_DIR=%HEADROOM_DIR%\workspace" "HF_HOME=%HEADROOM_HF_HOME%" "HEADROOM_INTERCEPT_READ_MIN_CHARS=4096" "HEADROOM_EFFORT_ROUTER=1" "HEADROOM_MECHANICAL_EFFORT=low" "HEADROOM_OUTPUT_SHAPER=1" "HEADROOM_PROTECT_TOOL_RESULTS=terminal,read_file,search_files,write_file,patch,Bash,Python,Read,Grep,Glob,Write,Edit,WebFetch"
...
timeout /t 3 /nobreak >nul 2>&1
"%CURL_HDR%" -s -m 8 -X POST http://127.0.0.1:8787/admin/runtime-env -H "Content-Type: application/json" -d "{\"HEADROOM_INTERCEPT_READ_MIN_CHARS\":\"4096\",\"HEADROOM_EFFORT_ROUTER\":\"1\",\"HEADROOM_MECHANICAL_EFFORT\":\"low\",\"HEADROOM_OUTPUT_SHAPER\":\"1\",\"HEADROOM_PROTECT_TOOL_RESULTS\":\"terminal,read_file,search_files,write_file,patch,Bash,Python,Read,Grep,Glob,Write,Edit,WebFetch\"}" >nul 2>&1
exit /b 0
