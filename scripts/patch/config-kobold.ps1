...
$baseUrlDone = $false
$defaultDone = $false
$contextFileDone = $contextFileExists  # Если уже есть — не вставляем!
$maxTokensDone = $false
...
for ($i = 0; $i -lt $lines.Count; $i++) {
    ...
    if ($inModelBlock) {
        ...
        # provider: auto/openrouter → custom
        if (-not $providerDone -and $trimmed -match '^provider:\s*"(auto|openrouter)"') {
        ...
        # Раскомментированная: "context_length: 131072"
        ...
        # Закомментированная: "# context_length: 131072"
        ...
        if ($insertAfterContextLength) {
            $result += $line
            # Вставляем context_file_max_chars ТОЛЬКО если:
            # 1. Нашли context_length ($foundContextLengthLine)
            # 2. Его ещё нет в файле (-not $contextFileDone)
