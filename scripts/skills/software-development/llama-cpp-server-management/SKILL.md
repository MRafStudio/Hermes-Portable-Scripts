...
- Flags for 5090 (sm_120): `--mmproj` (not -mmproj), `--flash-attn 1`, `--port` (not -p), `-ngl 999`, `--parallel 1`. CUDA 13.3 required (12.4 does not support Blackwell). **НЕ использовать `--image-min-tokens`**: для Gemma-4-26B он роняет загрузку mmproj (`image_max_pixels 645120 < image_min_pixels 2359296` — llama-server exiting due to model loading error); проверено биссекцией 14.08 — флаг убран из всех скриптов. **Думание per-model**: `--chat-template-kwargs {"enable_thinking":false}` — DEPRECATED (llama.cpp build 10425, warning при старте); правильный флаг `--reasoning off` (on/off/auto). Per-model управление через llama_models.py `thinking_flag` (файловый обмен: python → файл → `set /p` в .bat; никаких кавычек-танцев в cmd).
