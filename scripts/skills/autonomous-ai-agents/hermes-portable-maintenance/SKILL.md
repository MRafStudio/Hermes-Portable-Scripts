...
metadata:
  hermes:
    tags: [hermes, portable, backup, reinstall, windows, gitignore, installer, uv, npm, venv, robocopy]
...
- A file edited in the portable repo (e.g. a skill) does not show up in `data/` after an update
...
- `scripts/skills/` — **durable source of the user's own skills**, deployed to `%HERMES_HOME%\skills` by `robocopy "%SCRIPTS_DIR%\skills" "%HERMES_HOME%\skills" /E /XO` (line ~570 of the Desktop bat, ~545 of the Web bat). **A skill that must survive a wipe has to live here.** (Bundled upstream skills are a separate, re-synced set.)
...
An update can print `[OK] Installation Complete!` while four stages fail silently:
...
| **No log line at all** — a file edited in `scripts/skills/` just never appears/updates in `data/hermes/skills/` | the deploy used `robocopy ... /E /XC /XN /XO`. `/XO` (exclude older), `/XN` (exclude newer) and `/XC` (exclude changed) are mutually exclusive filters — together they match *every* file, so an existing destination file is NEVER overwritten and robocopy exits 0 with "no files copied". Only brand-new files land (which is why a fresh install looks fine) | use `robocopy ... /E /XO` — source newer ⇒ copy, destination newer ⇒ skip (keeps working-copy edits). Fixed in the Desktop/Web bats |
...
# deploy check: reproduce what the installer does, then look at the destination
robocopy "$SCRIPTS_DIR\skills" "$HERMES_HOME\skills" /E /XO /NFL /NDL /NJH /NJS   # exit 1 = copied, 0 = nothing to do
```
Do not trust: the printed "npm debug log" is the *probe's* (`npm config get cache`), not the failing install's; each failed npm run leaves `data/temp/hermes-npm-*.log` (cleanup only runs on the success branch); `вњ…` in output is UTF-8 read as cp866; **robocopy's exit code 0–7 means success, and `0` with "no files copied" is the silent-failure signature.**
...
- **Robocopy exclusions cancel each other out**: never combine `/XO /XN /XC` (or `/XO /XN`). Use `/XO` to mean "deploy when the repo copy is newer"; a "successful" exit 0 with no files copied is the tell. Test a deploy with a fixture before trusting it.
